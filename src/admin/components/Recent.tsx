...
import getRecent from "src/admin/queries/getrecently"
...
function Recent({ currentUser }) {
  ...
  var count = 0
  const [Recent] = useQuery(getRecent, {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-4 items-center justify-center p-5 w-full">
        {Recent?.map((project) => (
          <a
            key={project.id}
            className="rounded-xl p-5 text-lg drop-shadow-2xl"
            style={{ backgroundColor: nextColor(count++) }}
            onClick={() => {
          ...
          >
           ...
           c9.099,0,16.477,7.378,16.477,16.477V59.1C349.17,68.201,341.794,75.578,332.693,75.578z"
            ...
            <div className="block text-black items-center justify-center">
              <p className="text-md">Document Description:</p>
